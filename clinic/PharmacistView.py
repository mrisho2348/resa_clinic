...
from .models import AmbulanceOrder,ConsultationNotes, ConsultationOrder, Country, Diagnosis, Diagnosis, DiseaseRecode, ImagingRecord,InventoryItem, LabTest, LaboratoryOrder, MedicineInventory, NotificationMedicine, Order, PatientVisits, PatientVital, Prescription, Referral,Service, AmbulanceVehicleOrder
...
def save_remotereferral(request, patient_id, visit_id):
    try:
        ...
        return render(request, 'pharmacist_template/save_remotereferral.html', {
            'visit_history': visit_history,
            'patient': patient,
            'visits': visits,
            'range_31': range_31,
...
def notification_view(request):
    ...
    return render(request, 'pharmacist_template/manage_notification.html', context)
...
def ambulance_order_view(request):
    template_name = 'pharmacist_template/ambulance_order_template.html'
...
def save_ambulance_order(request, patient_id, visit_id, ambulance_id=None): 
    ...
    if request.method == 'POST':
        try:
            ...
            return render(request, 'pharmacist_template/add_ambulance_order.html', context)
    else:
...
def ambulance_order_detail(request, order_id):
    ...
    return render(request, 'pharmacist_template/ambulance_order_detail.html', {'ambulance_order': ambulance_order})
...
def vehicle_ambulance_view(request):
    ...
    template_name = 'pharmacist_template/vehicle_ambulance.html'
...
def manage_referral(request):
    ...
    patients = Patients.objects.all()
    return render(request, 'pharmacist_template/manage_referral.html', {'referrals': referrals,'patients':patients})
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if order_id:
                order = AmbulanceVehicleOrder.objects.get(pk=order_id)
                context['order'] = order
            return render(request, 'pharmacist_template/add_ambulance_carorder.html', context)
    except Exception as e:
...
def vehicle_detail(request, order_id):
    ...
    return render(request, 'pharmacist_template/vehicle_detail.html', {'order': order}) 
...
def new_lab_order(request):
    ...
    unread_orders = Order.objects.filter(order_type__in=[lab_record.name.name for lab_record in lab_records], is_read=True)     
    orders = unread_orders   
    unread_orders.update(is_read=True)     
...
def edit_disease_record(request, disease_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('pharmacist_manage_disease')  # Replace 'your_redirect_url' with the appropriate URL name

        except Exception as e:
            messages.error(request, f'An error occurred: {e}')

    return render(request, 'pharmacist_template/edit_disease.html', {'disease': disease})
...
def delete_disease_record(request, disease_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('pharmacist_manage_disease')  # Replace with the appropriate URL name

        except Exception as e:
            messages.error(request, f'An error occurred: {e}')

    return render(request, 'pharmacist_template/delete_disease_record_confirmation.html', {'record': record})
...
def medicine_list(request):
    ...
    return render(request, 'pharmacist_template/manage_medicine.html', {'medicines': medicines, 'unread_notifications': unread_notifications})   
...
def medicine_inventory_list(request):
    ...
    return render(request, 'pharmacist_template/manage_medical_inventory.html', context)
